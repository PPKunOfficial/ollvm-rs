...
bool Flattening::flatten(Function *f, const ObfOpt &opt) {
  ...
  switchVar = new AllocaInst(intType, 0, "switchVar", insert);
  if (pointerSize == 8) {
    new StoreInst(ConstantInt::get(intType, llvm::cryptoutils->scramble64(
                                                0, scrambling_key)),
                  switchVar, insert);
  } else {
    new StoreInst(ConstantInt::get(intType, llvm::cryptoutils->scramble32(
                                                0, scrambling_key)),
                  switchVar, insert);
  }
  ...
  for (vector<BasicBlock *>::iterator b = origBB.begin(); b != origBB.end();
    ...
    if (i->getTerminator()->getNumSuccessors() == 1) {
      ...
      if (numCase == NULL) {
        if (pointerSize == 8) {
          numCase = cast<ConstantInt>(ConstantInt::get(
              ...
              llvm::cryptoutils->scramble64(switchI->getNumCases() - 1,
                                            scrambling_key)));
        } else {
          numCase = cast<ConstantInt>(ConstantInt::get(
              switchI->getCondition()->getType(),
              llvm::cryptoutils->scramble32(switchI->getNumCases() - 1,
                                            scrambling_key)));
        }
      }
      ...
      Value *newNumCase =
          BinaryOperator::Create(Instruction::Sub, MySecret, X, "", i);
    ...
    if (i->getTerminator()->getNumSuccessors() == 2) {
      ...
      Value *newNumCaseTrue = BinaryOperator::Create(Instruction::Sub, MySecret,
                                                     X, "", i->getTerminator());
      Value *newNumCaseFalse = BinaryOperator::Create(
          Instruction::Sub, MySecret, Y, "", i->getTerminator());

      // Create a SelectInst
      BranchInst *br = cast<BranchInst>(i->getTerminator());
      SelectInst *sel =
          SelectInst::Create(br->getCondition(), newNumCaseTrue,
                             newNumCaseFalse, "", i->getTerminator());
